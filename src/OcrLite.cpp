...
void OcrLite::initModels(const std::wstring &detPath, const std::wstring &clsPath,
...
OcrResult OcrLite::detect( char * buffer,size_t bufferSize,
                          const int padding, const int maxSideLen,
                          float boxScoreThresh, float boxThresh, float unClipRatio, bool doAngle, bool mostAngle) {

	/*
	FILE* f = _wfopen(imgFile, L"rb");
	...
	*/
	...
	cv::Mat bgrSrc = cv::imdecode(vbuffer, cv::IMREAD_COLOR);
	...
	result = detect("log.disabled","log.disabled", paddingSrc, paddingRect, scale,
...
OcrResult OcrLite::detect(const char *path, const char *imgName,
    ...
    return OcrResult{dbNetTime, textBlocks, textBoxImg, fullTime, strRes,resultImageBuffer};
}